...
TYPES = {0x01: "Ping", 0x02: "SoftwareVersion", 0x03: "DeviceUID",
         0x05: "StorageSpace", 0x09: "Query", 0x53: "DataList"}


def build_request(type_byte, seq=1, cmd=0x04, ctr=0x00):
    """A device-0x10 request frame. Header after the literal is
    seq_hi seq_lo (u16) · 0x09 ctr (field16) · type."""
    seq_hi, seq_lo = (seq >> 8) & 0x7F, seq & 0x7F
    return bytes((*HDR, cmd, seq_hi, seq_lo, 0x09, ctr, type_byte, 0xF7))
...
def main():
    ...
    if args.type is not None:
        plan.append(args.type)
    if not plan:
        plan = [0x01]  # default: ping

    if args.dry_run:
        for i, t in enumerate(plan, 1):
            f = build_request(t, seq=i)
    ...
    try:
        for i, t in enumerate(plan, 1):
            replies = request(mi, mo, t, seq=i, wait=args.wait)
            if not replies:
                print("   <no reply>")
