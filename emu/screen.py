...
The panel is 128x64, 8bpp. `Bitmap` is not a plain buffer -- it carries a
`setPixel` function pointer at +0x0C, called as setPixel(Bitmap*, x, y, value).
So we never need to know how Bitmap stores pixels: we hand the firmware a
Bitmap whose setPixel points at a stub we intercept, and capture the calls.

    Bitmap     +0x04 width  +0x08 height  +0x0C setPixel fn ptr
    PixelData  +0x00 width  +0x04 height  +0x08 8bpp row-major buffer
...
STUB      = 0x30010000       # fake setPixel for renderers that use Bitmap+0x0C
...
class Screen:
    def __init__(self, main_img):
        ...
        def on_code(uc, addr, size):
            if addr in (STUB, SET_PIXEL):         # setPixel(this, x, y, value)
    ...
    def make_bitmap(self, w, h):
        self.m.ensure(BMP_ADDR)
        self.m.uc.mem_write(BMP_ADDR, struct.pack('>IIII', 0, w, h, STUB))
        return BMP_ADDR
